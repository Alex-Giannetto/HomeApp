//
//  AnimatedBlobComponent.swift
//  Home
//
//  Created by Perso on 19/02/2023.
//

import SwiftUI

struct AnimatedBlobComponent: View {
    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            Canvas { context, size in
                ...
                context.fill(path, with: .linearGradient(Gradient(colors: [.orange, .red]), startPoint: .init(x: 0, y: 0), endPoint: .init(x: 400, y: 400)))
            }
        }
...
struct AnimatedBlobComponent_Previews: PreviewProvider {
